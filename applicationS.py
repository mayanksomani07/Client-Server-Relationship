...
class Window(Frame):
	def connect(self):
		...
		self.log.config(text=self.log['text']+'\n'+"socket binded to port "+ str(port))
	...
	def sendMsg(self):
		...
		self.chat.append({'sender':'Server', 'content':content})
	...
	def threadedRecv(self):
		while True: 
			...
			if data:
					...
					self.chat.append({'sender':'Client', 'content':data.decode()})
...
if __name__ == '__main__': 
		root = Tk()
		app = Window(root)
		root.wm_title("Chat Application : Server")
